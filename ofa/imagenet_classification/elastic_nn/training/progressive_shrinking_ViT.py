...
def validate(
    ...
    if heads_list is None:
        heads_list = dynamic_net.heads_list
...
def train_one_epoch(run_manager, args, epoch, warmup_epochs=0, warmup_lr=0):
    dynamic_net = run_manager.network                                           #dynamic_net = OFANet (res/Mob/ViT)
    ...
    with tqdm(
        ...
        for i, (images, labels) in enumerate(run_manager.run_config.train_loader):
            ...
            for _ in range(args.dynamic_batch_size):
                ...
                subnet_settings = dynamic_net.sample_active_subnet()                    # sample a random subnet
                subnet_str += (                                                         # documenting subnet settings
                    "%d: " % _
                    + ",".join(
                        [
                            "%s_%s"
                ...
                output = run_manager.net(images)                                        # forward pass of an image
                ...
                loss.backward()                                                         # backpropagation for subnet
...
def load_models(run_manager, dynamic_net, model_path=None):
    ...
    dynamic_net.load_state_dict(new_state_dict, strict=True)
...
def train_elastic_depth(train_func, run_manager, args, validate_func_dict):
    ...
    if (
        len(set(dynamic_net.heads_list)) == 1
        and len(set(dynamic_net.width_mult_list)) == 1
    ):
...
def train_elastic_width_mult(train_func, run_manager, args, validate_func_dict):
    ...
    # load pretrained models
    if run_manager.start_epoch == 0 and not args.resume:
        validate_func_dict["width_mult_list"] = sorted(dynamic_net.width_mult_list)

        load_models(run_manager, dynamic_net, model_path=args.ofa_checkpoint_path)
        dynamic_net.re_organize_middle_weights(width_mult_stage=current_stage)
        run_manager.write_log(
            "%.3f\t%.3f\t%.3f\t%s"
            % validate(run_manager, is_test=True, **validate_func_dict),
            "valid",
    ...
    run_manager.write_log(
        ...
        % (
            ...
            width_stage_list[: current_stage + 2],
        )
        + "-" * 30,
        "valid",
    )
    if len(set(dynamic_net.heads_list)) == 1 and len(set(dynamic_net.depth_list)) == 1:
        validate_func_dict["width_mult_list"] = width_stage_list
    else:
        validate_func_dict["width_mult_list"] = sorted(
            {min(width_stage_list), max(width_stage_list)}
        )
